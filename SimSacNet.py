...
class SimSacNet(nn.Module):
    def __init__(self, evaluation, div=1.0, iterative_refinement=False,
                 ...
                 t_param='separate'):
        super(SimSacNet, self).__init__()
        
        self.tr_type = tr_type
        self.num_class = num_class
        self.pyramid = ResNetPyramid(train=True)
        self.t_param = t_param
        
        self.teacher_dec = SimSacDecoder(evaluation=evaluation, div=div, num_class=num_class,
                                         iterative_refinement=iterative_refinement, refinement_at_all_levels=refinement_at_all_levels,
                                         refinement_at_adaptive_reso=refinement_at_adaptive_reso, batch_norm=batch_norm, 
                                         pyramid_type=pyramid_type, md=md, upfeat_channels=upfeat_channels, 
                                         dense_connection=dense_connection,
                                         consensus_network=consensus_network, cyclic_consistency=cyclic_consistency, 
                                         decoder_inputs=decoder_inputs, use_pac=use_pac, 
                                         dense_cl=dense_cl, cl=cl, vpr_candidates=vpr_candidates)
        if self.tr_type == 'semi':
            self.student_dec = SimSacDecoder(evaluation=evaluation, div=div, num_class=num_class,
                                         iterative_refinement=iterative_refinement, refinement_at_all_levels=refinement_at_all_levels,
                                         refinement_at_adaptive_reso=refinement_at_adaptive_reso, batch_norm=batch_norm, 
                                         pyramid_type=pyramid_type, md=md, upfeat_channels=upfeat_channels, 
                                         dense_connection=dense_connection,
            ...
            if self.t_param == 'shared':
                del self.teacher_dec
                self.teacher_dec = self.student_dec
            elif self.t_param == 'ema':
                for param in self.teacher_dec.parameters:
                    param.requires_grad = False
                   
        
    def forward(self, src_image, tgt_image, src_image_256, tgt_image_256,
                aug_src_image=None, aug_tgt_image=None, aug_src_image_256=None, aug_tgt_image_256=None, test=False):
        out_dict = {}
        # features from original image
        im1_pyr, im2_pyr = self.pyramid(src_image, eigth_resolution=True), self.pyramid(tgt_image, eigth_resolution=True)
        im1_pyr_256, im2_pyr_256 = self.pyramid(src_image_256), self.pyramid(tgt_image_256)
        
        # stop gradient features if parameter of decoders are seperated
        if self.t_param == 'separate':
            im1_pyr_t, im2_pyr_t = [f.detach() for f in im1_pyr], [f.detach() for f in im2_pyr]
            im1_pyr_256_t, im2_pyr_256_t = [f.detach() for f in im1_pyr_256], [f.detach() for f in im2_pyr_256]
        else:
            im1_pyr_t, im2_pyr_t = im1_pyr, im2_pyr
            im1_pyr_256_t, im2_pyr_256_t = im1_pyr_256, im2_pyr_256
                    
        # teacher output
        if (self.tr_type == 'semi') and test:
            out_dict['teacher'] = None
        else:
            if self.t_param == 'shared':
                out_dict['teacher'] = self.student_dec(im1_pyr_t, im2_pyr_t, im1_pyr_256_t, im2_pyr_256_t)
            else:
                out_dict['teacher'] = self.teacher_dec(im1_pyr_t, im2_pyr_t, im1_pyr_256_t, im2_pyr_256_t)
        
        if self.tr_type == 'semi':
            if aug_src_image == None:
                # for labeled batch, student use pyramid from original image
                out_dict['student'] = self.student_dec(im1_pyr, im2_pyr, im1_pyr_256, im2_pyr_256)
            else:
                # for unlabeled batch, student use pyramid from augmented image
                aug_im1_pyr, aug_im2_pyr = self.pyramid(aug_src_image), self.pyramid(aug_tgt_image)
                aug_im1_pyr_256, aug_im2_pyr_256 = self.pyramid(aug_src_image_256), self.pyramid(aug_tgt_image_256)
                out_dict['student'] = self.student_dec(aug_im1_pyr, aug_im2_pyr, aug_im1_pyr_256, aug_im2_pyr_256)
        else: # Supervised or USL
            out_dict['student'] = None
            
        return out_dict
        
        
class SimSacDecoder(nn.Module):
    '''
    GLU-Net + change detection decoder
    '''
    ...
    def __init__(self, evaluation, div=1.0, iterative_refinement=False,
        ...
        """
        super(SimSacDecoder, self).__init__()
    ...
    def forward_sigle_ref(self, im1_pyr, im2_pyr, im1_pyr_256, im2_pyr_256, im_target=None, im_source=None, disable_flow=None):
        # this function warps im1 --> im2
        disable_flow = None
        
        # extract pyramid features
        # (channel,height): (64,520) -> (128,130) -> (256,65)
        # if self.t_or_s == 'teacher':
        #     for im_pyr in [im1_pyr, im2_pyr, im1_pyr_256, im2_pyr_256]:
        #         for i, fmap in enumerate(im_pyr):
        #             im_pyr[i] = fmap.detach()
        ...
        change3 = self.change_dec3(x1=c23, x2=c13_w, x3=corr3_cnghead, mask=up_change4_binary)
        ...
        change2 = self.change_dec2(x1=c22, x2=c12_w, x3=corr2_cnghead, mask=up_change3_binary)
        if self.use_pac:
            aligned_imgs_2 = self.resize_align_images(im_source, im_target, size=(int(h_full / 4.0), int(w_full / 4.0)),
                                                      flow=up_flow3*div*ratio)
            up_change2 = self.change_deconv2(change2,aligned_imgs_2)
        ...
        change1 = self.change_dec1(x1=c21, x2=c11_w, x3=corr1_cnghead, mask=up_change2_binary)
    ...
    def forward(self, im1_pyr, im2_pyr, im1_pyr_256, im2_pyr_256):
        if self.vpr_candidates:
            return self.forward_multiple_ref(im1_pyr, im2_pyr, im1_pyr_256, im2_pyr_256)
        else:
            return self.forward_sigle_ref(im1_pyr, im2_pyr, im1_pyr_256, im2_pyr_256)
